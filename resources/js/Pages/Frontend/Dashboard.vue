                ...
                <div class="container mx-auto flex items-center flex-wrap pt-4 pb-12">
                    <div class="w-full md:w-1/3 xl:w-1/4 p-6 flex flex-col" v-for="(product, index) in products.data"
                        ...
                        <inertia-link :href="$route('product', {product : product.slug})">
                            <div class="h-48">
                                <img class="max-h-full object-cover hover:grow hover:shadow-lg"
                                     :src="product.main_image_path" :alt="product.name">
                            </div>
...
export default {
    ...
    computed: {
        issetRecentViews() {
            return this.recentViews.length > 0
        }
    },

    async created() {
        try {
            let list = await axios.get(this.$route('recent-views'))
            this.recentViews = list.data
        } catch (e) {
            console.error('Could not find recent views data')
        }
    }
}
</script>
